...
gpxParser.prototype.parse = function (gpxstring) {
    ...
    for (let idx in trks){
        ...
        let roughnesses = keepThis.getElementValue(trk, "roughness");
        if (roughnesses !== null){
            roughnesses = parseRoughness(roughnesses);
        }
        
        for (let seg in trksegs){
            let segment = trksegs[seg];
            let trkpts = [].slice.call(segment.querySelectorAll('trkpt'));
            ...
            for (let idxIn in trkpts){
                ...
                let floatValue = parseFloat(trkpt.getAttribute("ele")); 
                let floatValue2 = parseFloat(keepThis.getElementValue(trkpt, "ele")); 

                pt.ele = isNaN(floatValue) ? floatValue2 : floatValue;
    
                let time = keepThis.getElementValue(trkpt, "time");
                pt.time = time == null ? null : new Date(time);
                pt.seg = seg;

                // store additional modified data
                pt.dist = parseFloat(trkpt.getAttribute("distance"));
                pt.bear = parseFloat(trkpt.getAttribute("bearing"));
                pt.slope = parseFloat(trkpt.getAttribute("slope"));
                pt.slope_per = parseFloat(trkpt.getAttribute("slope_percent"));

                if (roughnesses !== null){
                    pt.rough = roughnesses[seg];
                } else {
                    pt.rough = 0;
                }
...
gpxParser.prototype.toGeoJSON = function () {
    ...
    for(const idx in this.tracks) {
        ...
        for(const id in track.points) {
            let pt = track.points[id];
...
// function to parse roughness array
function parseRoughness(object)
{
    object = object.slice(1,object.length-1);
    var objects = object.split(",");
    var data = objects.map(item => {
        return +item;
    })
    return data;
}
